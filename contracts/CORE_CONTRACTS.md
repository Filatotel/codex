...
## ASSIGNMENT
**Definition:** exact instruction for what must happen now. **Authority:** issuer within declared scope. **Required fields:** id, objective, authority, input state ref, allowed/forbidden scope, acceptance, required outputs, stop conditions, result recipient, exact destination, capability profile ref, admissibility ref, required capabilities, mandatory evidence paths, and `execution_contract.proof_status=PROVEN`. **Invariants:** assignment is bounded; missing authority is not inferred; **NO ASSIGNMENT WITHOUT EXECUTABILITY PROOF**. If destination executability is not proven, the artifact remains an assignment draft/control decision and must not be issued as executable work. When `required_durable_outputs` is present, each entry is an opaque assignment-local output identity. `EXECUTOR_RESULT.status=COMPLETE` is valid only when every such identity has exactly one non-blank binding in `EXECUTOR_RESULT.durable_output_refs`. These bindings establish reference coverage only: **DURABLE REF PRESENT != DURABLE READBACK PROVEN**. **Do not confuse with:** role contract, project Canon, or an `ASSIGNMENT_NOT_ADMISSIBLE` control outcome.

## ARTIFACT
**Definition:** durable carrier of a result, claim, decision, or state proposal. **Authority:** source role produces it; downstream authority decides acceptance where required. **Required fields:** stable identity, type, producer role, assignment/state refs, status, provenance, related artifacts. **Invariant:** ARTIFACT != EVIDENCE. A durable-output reference naming an artifact does not by itself prove that the artifact is resolvable, readable, fresh, or independently consumed. **Do not confuse with:** proof simply because it exists.
...
- `DURABLE REF PRESENT != DURABLE READBACK PROVEN`: A1 reference completeness is not materialization/readback evidence.
